...
import { PenTool, Image, Video, FileText, Share2, Code, MessageCircle, Music, Languages, FileSearch, Presentation, ArrowRight, ImagePlus } from 'lucide-react';
...
export default async function ToolsPage({
  ...
  const tools = [
    ...
    },
    {
      id: 'image-to-code',
      icon: ImagePlus,
      href: `/${validLocale}/tools/image-to-code`,
      title: (t['imageToCode'] as Record<string, string>)?.title || 'Image to Code',
      description: (t['imageToCode'] as Record<string, string>)?.subtitle || 'Convert design screenshots into code',
      color: 'accent-glow',
    },
    {
      id: 'video',
        ...
        <div className="container-custom">
          ...
          <div className="mx-auto grid max-w-6xl gap-6 md:grid-cols-3 lg:grid-cols-4">
            {tools.map((tool) => (
              ...
              >
                <div className={`mb-4 flex h-12 w-12 items-center justify-center rounded-xl ${colorClasses[tool.color].bg}`}>
                  <tool.icon className={`h-6 w-6 ${colorClasses[tool.color].text}`} />
                </div>
